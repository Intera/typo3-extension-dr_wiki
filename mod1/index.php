				...
				if($cmd){
					switch($cmd){
						...
						case "delete_other":
							...
							$rename_submit = t3lib_div::GPVar('rename_submit');
							$error = '';
							if (!empty ($rename_submit)) {
								$rename_to= t3lib_div::GPVar('rename_to');
								if (empty ($rename_to)) {
								...
								if (!$GLOBALS['TYPO3_DB']->sql_error()){
									...
									// debugster ($count['COUNT(*)']);
									if  ($count['COUNT(*)'] > 0) {
										$error = $LANG->getLL ("rename_keywordexists");
							...
							if (empty ($error))
							{
								$alter_bodies = t3lib_div::GPVar('alter_body');
								$now = time();

								if (!empty ($alter_bodies))
								{
									// Change existing links to new keyword
									foreach ($alter_bodies as $alter_keyword)
									{
										...
										// Change Links
										$revision['body'] = str_ireplace ("[[$keyword]]","[[$rename_to]]",$revision['body']);
										$revision['body'] = str_ireplace ("[[$keyword|","[[$rename_to|",$revision['body']);

										// Submit new Version of body
										$insertArray = 	array (
										'pid' => $revision['pid'],
										'tstamp' => $now,
										'crdate' => $now,
										...
										'summary' => "Changed links to $keywword to $rename_to");
								...
								// Change all revision to new keyword
								$query4 = $GLOBALS['TYPO3_DB']->UPDATEquery('tx_drwiki_pages', "keyword='$keyword'", array ('keyword' => $rename_to));
								$res = $GLOBALS['TYPO3_DB']->sql_query ($query4);
								if (!$GLOBALS['TYPO3_DB']->sql_error()){
									//		debug(array($GLOBALS['TYPO3_DB']->sql_error(),$query));
								}
...
#									$query2 = "select * FROM tx_drwiki_pages where keyword = '$rename_to' ORDER BY `crdate` DESC LIMIT 0,1";
							...
							{
								$contentRename .= "<div style='background-color: #E00000 ;border-width: 1px; color:white;border-color: black; border-style:solid;margin:5px;padding:5px'>$error</div>";
							}
							}

							$contentRename .= $LANG->getLL("rename_to").' <input name="rename_to" type="text" value="'.t3lib_div::GPVar('rename_to').'"> <br>';
							$contentRename .= $LANG->getLL("rename_redir").' <input type="checkbox" name="create_redir" value="on"';
							$contentRename .= (t3lib_div::GPVar('create_redir') == 'on') ? "checked='checked'" : "";
							$contentRename .= '><br>';
							$contentRename .= '<input style="margin:10px 0px 6px 0px" type="submit" name="rename_submit" value="'.$LANG->getLL("update").'""> ';
							//debugster ($LANG->getLL("rename_accept"));
							//debugster (t3lib_div::GPVar('alter_body'));
							//Select all keywords, which ever had the keyword in question in their body
							$query = "select distinct keyword FROM tx_drwiki_pages where ( ( body like '%[[$keyword]]%') OR ( body like '%[[$keyword|%') )";
							$res = $GLOBALS['TYPO3_DB']->sql_query($query);
							//debugster ($query);
							if (!$GLOBALS['TYPO3_DB']->sql_error()){
								//debug(array($GLOBALS['TYPO3_DB']->sql_error(),$query));
							}
							$contentRename.="<div style='font-size:1.0em;font-weight:bold;margin:10px 0 10px 0'>".$LANG->getLL("rename_alterbodys")."?</div>";
...
#								$query2 = "select * FROM tx_drwiki_pages where keyword = '$occurence' ORDER BY `crdate` DESC LIMIT 0,1";
								...
								$body = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res2);
								if( (stristr($body['body'],"[[$keyword]]")) OR ( stristr($body['body'],"[[$keyword|")))
								{
									//debugster ($body["keyword"]);
									$contentRename .= "<div style='font-size:1.0em;font-weight:bold;margin-top:10px'><input type='checkbox' name='alter_body[]' value='{$body["keyword"]}'";
									if (is_array (t3lib_div::GPVar('alter_body') ))
									{
										$contentRename .= (in_array($body["keyword"],t3lib_div::GPVar('alter_body'))) ? "checked='checked'" : "";
									}
									$contentRename .= ">  {$body["keyword"]}</div>";
									// Show all occurences in the text and mark it in bold
									//debugster ($body['body']);
									if (preg_match_all ("§\s{0,1}.{0,25}\[\[$keyword\]\].{0,25}\s{0,1}+§i",$body['body'],$matches))
									{
							...
							$contentRename .= '<input style="margin:10px 0px 6px 0px" type="submit" name="rename_submit" value="'.$LANG->getLL("update").'""> ';
							$contentRename.="<span style='padding-bottom:12px'>&nbsp;</span>";
							break;
					}
				}
				// get all keywords
				...
				while(list(,$keyword) = each($keywords)){
					$contentKeyword .= '<tr><td valign="top"><a href="index.php?id=' . $this->id . '&tx_drwiki_mod1[keyword]='.$keyword.'#versions">'.$keyword.'</a></td><td>&nbsp;<a href="index.php?id=' . $this->id . '&tx_drwiki_mod1[keyword]='.$keyword.'#versions"><img src="../res/mod1/view.png" border="0" title="'.$LANG->getLL("alt_VIEW").'" alt="'.$LANG->getLL("alt_VIEW").'"></a>&nbsp;<a href="index.php?id=' . $this->id . '&tx_drwiki_mod1[keyword]='.$keyword.'&tx_drwiki_mod1[cmd]=check_delete_keyword"><img src="../res/mod1/delete.png" border=0 title="'.$LANG->getLL("alt_DELETE").'" alt="'.$LANG->getLL("alt_DELETE").'">&nbsp;<a href="index.php?id=' . $this->id . '&tx_drwiki_mod1[keyword]='.$keyword.'&tx_drwiki_mod1[cmd]=rename"><img src="../res/mod1/rename.png" border=0 title="'.$LANG->getLL("alt_RENAME").'" alt="'.$LANG->getLL("alt_RENAME").'"></td></tr>';
				}
				$contentKeyword .= "</table>";
				...
				if($keyword){
					...
					if (!empty ($contentRename)) {
						$this->content.=$this->doc->section($LANG->getLL("rename_title").$keyword,$contentRename,0,1);
					}
